...
export async function getRoleList(params: PageParams): Promise<ApiResponse<PageResult<Role>>> {
  ...
  if (name)
    filtered = filtered.filter((r) => r.name?.toLowerCase().includes(String(name).toLowerCase()));
  if (code)
    filtered = filtered.filter((r) => r.code?.toLowerCase().includes(String(code).toLowerCase()));
