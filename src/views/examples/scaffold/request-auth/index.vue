...
async function mockProtectedApi(apiName: string) {
...
async function requestWithAutoRefresh(apiName: string) {
  try {
...
async function handleRefreshFailScenario() {
  expireAccessToken()
  ...
  <div class="page-container">
    <div class="card">
      ...
      <a-space wrap :size="10" class="mb-lg">
        <a-button type="primary" @click="handleSingleRequest">
          {{ $t('examples.scaffold.requestAuth.singleRequest') }}
        </a-button>
        <a-button @click="handleConcurrentRequest">
          {{ $t('examples.scaffold.requestAuth.concurrentRequest') }}
        </a-button>
        <a-button @click="expireAccessToken">
          {{ $t('examples.scaffold.requestAuth.expireToken') }}
        </a-button>
        <a-button danger @click="handleRefreshFailScenario">
          {{ $t('examples.scaffold.requestAuth.simulateRefreshFail') }}
        </a-button>
        <a-button @click="resetSession">
          {{ $t('examples.scaffold.requestAuth.resetSession') }}
        </a-button>
      </a-space>

      <div class="log-toolbar">
        <span class="text-secondary">{{ $t('examples.scaffold.requestAuth.logTitle') }}</span>
        <a-button size="small" @click="logs = []">
          {{ $t('examples.scaffold.requestAuth.clearLog') }}
        </a-button>
      </div>

      <div class="log-list">
        <div v-if="logs.length === 0" class="log-empty">
          {{ $t('examples.scaffold.requestAuth.noLogs') }}
        </div>
