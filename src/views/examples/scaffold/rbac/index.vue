...
function maskText(value: string, reserve = 2) {
  if (!value) {
    return '-'
  }
...
async function switchAccount(username: 'admin' | 'user') {
  try {
...
async function callApiWithPermission(permission: string, actionName: string) {
  try {
...
<template>
  <div class="page-container">
    ...
    <div class="card mb-lg">
      ...
      <div class="tags-wrap">
        <a-tag v-for="perm in displayPermissions" :key="perm">
          {{ perm }}
        </a-tag>
      </div>
    </div>
    ...
    <div class="grid-two">
      <div class="card">
        <div class="title mb-sm">
          {{ $t('examples.scaffold.rbac.section1Title') }}
        </div>
      ...
      <div class="card">
        <div class="title mb-sm">
          {{ $t('examples.scaffold.rbac.section2Title') }}
        </div>
        <a-space wrap>
          <PermissionButton permission="system.user.create">
            <a-button type="primary">
              {{ $t('examples.scaffold.rbac.createUserButton') }}
            </a-button>
          </PermissionButton>
          ...
          <PermissionButton permission="system.user.delete">
            <a-button danger>
              {{ $t('examples.scaffold.rbac.deleteUserButton') }}
            </a-button>
          </PermissionButton>
        </a-space>
      ...
      <div class="card">
        <div class="title mb-sm">
          {{ $t('examples.scaffold.rbac.section3Title') }}
        </div>
        <div class="field-row">
